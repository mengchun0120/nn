...
LIBS=-lm
COMMON_FLAGS=-std=c++11 -Wall -fpic -I$(SRC_DIR) -I$(TEST_TOOL_DIR) -L$(BIN_DIR)
...
$(TEST_TOOL_BIN_DIR)/%.o: $(TEST_TOOL_DIR)/%.cpp $(TEST_TOOL_DIR)/%.hpp $(OBJS)
	g++ $(CPPFLAGS) -c -o $@ $<
...
clean:
	rm -f $(BIN_DIR)/* $(UNIT_TEST_BIN_DIR)/* $(TEST_TOOL_BIN_DIR)/*
